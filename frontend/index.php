...
<body>
    ...
    <div id="mesas" class="pantalla">
        ...
        <div style="padding: 16px 32px 0 32px; display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
            ...
            <button class="btn" id="btn-mensajes" onclick="abrirPanelMensajes()" style="background:#1A5A8A; color:#fff; position:relative;">
                <i class="fas fa-bell"></i> MENSAJES
                <span id="badge-mensajes" style="display:none; position:absolute; top:-6px; right:-6px; background:#E53935; color:#fff; border-radius:50%; width:18px; height:18px; font-size:10px; font-weight:800; line-height:18px; text-align:center;"></span>
            </button>
...
<div id="notif-pos" class="notif-pos" style="display:none;">
    ...
    <div id="modal-reserva-general" class="modal-overlay">
        <div class="modal-caja" style="max-width:900px; padding:0; overflow:hidden; border-radius:16px; max-height:92vh; display:flex; flex-direction:column;">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:0; overflow-y:auto; flex:1; min-height:0;">

                <!-- Columna izquierda: Mesas + calendario -->
                <div style="padding:20px; padding-right:16px; border-right:1px solid var(--border-color,#D4E0CF); position:relative; z-index:1;">
                    ...
                    <div id="grv-mesas-grid" style="display:grid; grid-template-columns:repeat(3,1fr); gap:8px; margin-bottom:20px; margin-right:4px;"></div>
                ...
                <div style="padding:20px; display:flex; flex-direction:column; gap:14px; position:relative; z-index:0;">
                    <!-- Resumen selección -->
    ...
    <!-- ══════════════════════════════════════════════════════════
         PANEL DE MENSAJES — notificaciones de cocina y barra
    ══════════════════════════════════════════════════════════ -->
    <div id="panel-mensajes" style="display:none; position:fixed; top:0; right:0; width:360px; max-width:100vw; height:100vh; background:var(--bg-card,#fff); border-left:1px solid var(--border-color,#D4E0CF); box-shadow:-8px 0 32px rgba(0,0,0,.18); z-index:3000; display:none; flex-direction:column;">
        <!-- Header -->
        <div style="display:flex; align-items:center; justify-content:space-between; padding:18px 20px; border-bottom:1px solid var(--border-color,#D4E0CF); background:var(--bg-app,#F0F4EE);">
            <div style="display:flex; align-items:center; gap:10px;">
                <i class="fas fa-bell" style="color:#1A5A8A; font-size:18px;"></i>
                <div>
                    <div style="font-size:15px; font-weight:800; color:var(--text-main,#1C2B18);">Mensajes</div>
                    <div style="font-size:11px; color:var(--text-muted,#6B7F65);">Cocina y Barra → Mesero</div>
                </div>
            </div>
            <div style="display:flex; gap:8px; align-items:center;">
                <button onclick="limpiarMensajesLeidos()" style="background:transparent; border:1px solid var(--border-color,#D4E0CF); border-radius:7px; padding:5px 10px; font-size:11px; font-weight:600; cursor:pointer; color:var(--text-muted,#6B7F65);">
                    <i class="fas fa-check-double"></i> Limpiar
                </button>
                <button onclick="cerrarPanelMensajes()" style="background:transparent; border:none; font-size:18px; cursor:pointer; color:var(--text-muted,#6B7F65); padding:4px 8px;">
                    <i class="fas fa-times"></i>
                </button>
            </div>
        </div>
        <!-- Lista mensajes -->
        <div id="lista-mensajes" style="flex:1; overflow-y:auto; padding:12px; display:flex; flex-direction:column; gap:10px;">
            <div style="text-align:center; padding:40px 0; color:var(--text-muted,#6B7F65);">
    ...
    <!-- Overlay para cerrar panel mensajes -->
    <div id="overlay-mensajes" onclick="cerrarPanelMensajes()" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.3); z-index:2999;"></div>
        ...
        try {
            ...
            if(localStorage.getItem('admin_ir_mesas')==='1'){
                ...
                if(sesAdmin && sesAdmin.rol === 'admin') {
                    ...
                    if(btnV) btnV.classList.remove('visible');
                }
            }
    ...
    (function(){
        try {
            ...
            // Botón Volver a Admin — siempre oculto al cargar
            // Solo se muestra cuando admin llama adminVerMesas() explícitamente
